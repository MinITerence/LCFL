...
from data_utils import DatasetSplit, get_dataset_mnist
from torch.utils.data import DataLoader
import pickle


# python main_mnist.py --usernum 100 --frac 0.32 --chunksize 10 --datadir ~/datasets --cr 100
...
class Server:
    def __init__(self, chunksize, clients, testset, log_name):
        self.model_global = NetCNN(chunksize) # DNN to train
...
def main():
    ...
    clients = []
    train_dataset, test_dataset, user_groups = get_dataset_mnist(data_dir, iid, num_users)
    Client.dataset = train_dataset
    for user_idx in range(num_users):
        clients.append(Client(user_groups[user_idx], user_idx))
    
    log_name = 'MNIST_usernum={}_frac={}_chunksize={}_cr={}_iid={}'.format(num_users, frac, chunksize, comm_round, iid)
