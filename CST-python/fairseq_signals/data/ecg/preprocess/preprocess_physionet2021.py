...
def get_parser():
    ...
    parser.add_argument("--root", 
                        default= '/data/mcc_fj/Match/physionet.org/files/challenge-2021/1.0.3/training',
                        metavar="DIR",
                       help="root directory containing mat files to pre-process")
    parser.add_argument(
        "--subset",
        default="WFDB_chapman_shaoxing",
        type=str,
    ...
    parser.add_argument("--rawpath", 
                        default= '/data/mcc_fj/Match/physionet.org/files/challenge-2021/1.0.3/training/WFDB_chapman_shaoxing',
                        metavar="DIR",
                       help="Raw data directory containing mat files to pre-process")
    parser.add_argument("--meta-dir",
                        default= '/home/mcc_fj/code/CHIL/ecg_federated',
    ...
    parser.add_argument("--workers",  default=2, metavar="N", type=int,
...
def main(args):
    ...
    for i, fname in enumerate(glob.iglob(os.path.join(args.rawpath, "**/*."+args.ext))):
        pid_table[os.path.basename(fname)[:-4]] = i
    ...
    for s in subset:
        ...
        print(len(fnames)) #CPSC2018=6877
        chunk_size = math.ceil(len(fnames) / args.workers)
        print(chunk_size)  #CPSC2018=3439

        file_chunks = [fnames[i:i+chunk_size] for i in range(0, len(fnames), chunk_size)]
        #print(file_chunks)
        print(len(file_chunks))    #CPSC2018=2
        print(len(file_chunks[1]))  #CPSC2018=len(file_chunks[0])=3439   CPSC2018=len(file_chunks[0])=3438
